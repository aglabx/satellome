...
int main(int argc, char **argv)
{
    ...
    if (trs_n_items < SIZE_WHILE_COMPUTE_ALL_PAIRS)
    {
        ...
        run_function_with_threads2D<std::function<void(size_t,
                                    ...
                                    const std::vector<TR> &,
                                    std::vector<TR> &,
                                    std::vector<Distance> &>(dist_n_items,
                                                             num_threads,
    ...
    else if (!skip_distances)
    {
        ...
        run_function_with_results_vector<std::function<void(size_t,
                                         ...
                                         const std::vector<TR> &,
                                         std::vector<TR> &,
                                         std::vector<std::vector<Distance>> &>(dist_n_items,
                                                                               num_threads,
    ...
    for (size_t i = 0; i < trs_vector.size(); i++)
    {
        ...
        fout << trs_vector[i].tr_array.size() << "\t";
        fout << trs_vector[i].tr_mono.size() << "\t";
        for (size_t j = 0; j < trs_vector[i].embeding.size(); j++)
        {
            fout << trs_vector[i].norm_embeding[j] << "\t";
        }
        fout << std::endl;
    }
    fout.close();
