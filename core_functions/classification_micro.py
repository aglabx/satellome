...
def _trs_separate_something(input_trf_file, 
                                                           output_trf_file, 
                                                           output_gff_file, 
                                                           filter_func, 
                                                           name_func, 
        ...
        with open(output_gff_file, "w") as fh_gff:
            ...
            for i, trf_obj in enumerate(trf_objs):
                if filter_func(trf_obj):
                    ...
                    stats[trf_obj.trf_family].max_length = max(stats[trf_obj.trf_family].max_length, trf_obj.trf_array_length)
                    stats[trf_obj.trf_family].min_length = min(stats[trf_obj.trf_family].max_length, trf_obj.trf_array_length)
...
def cf_separate_perfect_microsatellites(settings, project):
    ...
    r = _trs_separate_something(trf_all_file, trf_pmicro_file, gff_pmicro_file, filter_func, name_func, family_table_file=family_table_file)
    dataset = project["work_files"]["ref_assembly_name_for_trf"]
    n = r["filtered"]
    pgenome = round(100.*float(r["total_length"])/project["work_files"]["assembly_stats"][dataset]["total_length"], 3)
...
def cf_separate_microsatellites(settings, project):
    ...
    if "ref_dataset" in project:
        trf_all_file = os.path.join(trf_parsed_folder, project["ref_dataset"] + ".trf_all.trf")
    else:
        trf_all_file = settings["files"]["trf_all_file"]
    ...
    r = _trs_separate_something(trf_all_file, trf_micro_file, gff_micro_file, filter_func, name_func, family_table_file=family_table_file)
    dataset = project["work_files"]["ref_assembly_name_for_trf"]
    n = r["filtered"]
    pgenome = round(100.*float(r["total_length"])/project["work_files"]["assembly_stats"][dataset]["total_length"], 3)
...
def cf_separate_true_ssr(settings, project):
    ...
    r = _trs_separate_something(trf_all_file, trf_file, gff_file, filter_func, name_func, family_table_file=family_table_file)
    dataset = project["work_files"]["ref_assembly_name_for_trf"]
    n = r["filtered"]
    pgenome = round(100.*float(r["total_length"])/project["work_files"]["assembly_stats"][dataset]["total_length"], 3)
...
def cf_separate_complex_trs(settings, project):
    ...
    trf_complex_file = settings["files"]["trf_complex_file"] % project["work_files"]["ref_assembly_name_for_trf"]
    gff_complex_file = settings["files"]["gff_complex_file"] % project["work_files"]["ref_assembly_name_for_trf"]
    clouds_complex_file = settings["files"]["clouds_complex_file"] % project["work_files"]["ref_assembly_name_for_trf"]
    clouds_png_complex_file = settings["files"]["clouds_png_complex_file"] % project["work_files"]["ref_assembly_name_for_trf"]

    filter_func = lambda x: len(x.trf_consensus) > 4 and x.trf_array_length > 100 and x.trf_pmatch < 100.0 \
            and x.trf_array_gc > 0.2 and x.trf_array_gc < 0.8 \
            and x.trf_n_copy > 4 and x.trf_entropy > 1.82
    ...
    def name_func(trf_obj):
        ...
        d = (trf_obj.trf_period, trf_obj.trf_array_gc, trf_obj.trf_pmatch, trf_obj.trf_family)
        mathstr = "%s\n" % "\t".join(map(str, d))
        return trf_obj.trf_family, gff, mathstr

    r = _trs_separate_something(trf_all_file, 
                trf_complex_file, 
                gff_complex_file, 
                filter_func, 
                name_func, 
...
def cf_separate_1kb(settings, project):
    ...
    r = _trs_separate_something(trf_all_file, trf_file, gff_file, filter_func, name_func)
...
def cf_compute_micro_kmers(settings, project):
    ...
    index = compute_kmer_index_for_trf_file(trf_micro_file, trf_micro_kmers_file, k=23, max_complexity=None, min_complexity=None)
...
def cf_compute_perfect_micro_kmers(settings, project):
    ...
    index = compute_kmer_index_for_trf_file(trf_pmicro_file, trf_micro_kmers_file, k=23, max_complexity=None, min_complexity=None)
...
def cf_get_micro_summary_table(settings, project):
    ...
    output_tsv = os.path.join("/home/akomissarov/Dropbox/PySatDNA/reports", project["pid"], "microsatellites.summary.tsv")
    
    report_folder = os.path.join("/home/akomissarov/Dropbox/PySatDNA/reports", project["pid"])
    ...
    for name in keys:
        s = (
                    ...
                    micro_s[name],
                    lengths[name],
                    round(100.*nmicro_s[name]/micro_s[name], 2),
                    round(100.*pmicro_s[name]/micro_s[name], 2),
                )
        data.append(s)
