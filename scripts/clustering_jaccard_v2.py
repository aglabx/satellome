    ...
    for i in range(len(sequences_names)):
        for j in range(len(k_mers)):
            if k_mers[j] in sequences[sequences_names[i]]:
                matrix[i][j] = 1
    distance_matrix = pairwise_distances(matrix, metric="jaccard")
    return distance_matrix, sequences_names


def write_output(out: click.Path(), clusters_dict: dict, time, k, mode_value) -> None:
    with open(out, "w") as file:
        file.write(f"Run time: {time}\n")
        file.write(f"Parametrs: k - {k}, threshold - {mode_value}\n")
        file.write("Clusters:\n")
        for i in sorted(clusters_dict.keys()):
            file.write(f"Cluster #{i+1}: {','.join(clusters_dict[i])}\n")
...
def cluster(
    fasta: click.Path(), k: int, mode: str, mode_value: str, out: click.Path()
) -> None:
    start = datetime.now()
    matrix, sequences_names = make_matrix(fasta, k)
    if mode == "clust":
        clustering = AgglomerativeClustering(
            n_clusters=int(int(mode_value)), affinity="precomputed", linkage="average"
        ).fit(matrix)
    else:
        clustering = AgglomerativeClustering(
